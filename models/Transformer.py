...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # Decoder(异常检测的decoder被全连接层替换掉，因为是在重建序列)
    ...
    def anomaly_detection(self, x_enc):  # x_enc (32,96,25)
        # Embedding
        enc_out = self.enc_embedding(x_enc, None) # enc_out (32,96,64)
        enc_out, attns = self.encoder(enc_out, attn_mask=None) # enc_out (32,96,64)
        dec_out = self.projection(enc_out) # dec_out (32,96,25)
        return dec_out
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        if self.task_name == 'anomaly_detection':
            dec_out = self.anomaly_detection(x_enc)  # (32,96,25)
            return dec_out  # [B, L, D]
        if self.task_name == 'classification':
