...
class SCIBlock(nn.Module):
    def __init__(self, d_model, kernel_size=5, dropout=0.0):
        ...
        ###蹦出来4个不同的卷积模块吧，对应rou rou eta phai四个模块

    def forward(self, x):
        x_even, x_odd = self.splitting(x)
        x_even = x_even.permute(0, 2, 1) ### bfs
        ...
        return x_even_update.permute(0, 2, 1), x_odd_update.permute(0, 2, 1)### bsf
...
class SCINet(nn.Module):
    ...
    def zip_up_the_pants(self, even, odd):
        ...
        for i in range(min_len):
            zipped_data.append(even[i].unsqueeze(0))###even的第0个step，取出来，这里只有b和f
            zipped_data.append(odd[i].unsqueeze(0))###odd的第0个step
        if even_len > odd_len:
            zipped_data.append(even[-1].unsqueeze(0))###
        return torch.cat(zipped_data,0).permute(1, 0, 2)###沿时间方向拼起来，之后变成bsf
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.num_stacks = configs.d_layers ###这里是1
