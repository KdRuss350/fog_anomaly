...
class M4Dataset:
    ...
    @staticmethod
    def load(training: bool = True, dataset_file: str = '../m4') -> 'M4Dataset':
        """
        Load cached dataset.
        ...
        return M4Dataset(ids=m4_info.M4id.values,###Y1 Y2 这些
                         groups=m4_info.SP.values,### Yearly这些
                         frequencies=m4_info.Frequency.values,### 1 4 14 24这些
                         horizons=m4_info.Horizon.values,### horizon这些
                         values=np.load( ### 训练和测试文件
...
@dataclass()
class M4Meta:
    seasonal_patterns = ['Yearly', 'Quarterly', 'Monthly', 'Weekly', 'Daily', 'Hourly']###简称sp
